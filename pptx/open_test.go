...
func TestOpenMinimalPresentation(t *testing.T) {
	data := buildMinimalPPTX(t)
	pr, err := Open(bytes.NewReader(data), int64(len(data)))
	if err != nil {
		t.Fatal(err)
	}
	if pr.MainPart() != "/ppt/presentation.xml" {
		t.Fatalf("main: %q", pr.MainPart())
	}
	if err := pr.Write(&bytes.Buffer{}); err != ErrNotImplemented {
